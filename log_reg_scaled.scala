...
import math.floor
    ...
    val triplets = text.map{_.split(",")}.map{x => (x(0).toInt, x(1).split(' ').map(_.toDouble), x(2))}.map{case (label, arr, st) => (label, arr, arr.reduceLeft(_ min _), arr.reduceLeft(_ max _), st)}.map{case (label, arr, arrmin, arrmax, st) => (label, arr.map{x => if (arrmin == arrmax) {0} else {(x - arrmin)/(arrmax - arrmin)}}, st)}
