...
import math.floor
    ...
    val triplets = text.map(_.split(",")).map(x => (x(0).toInt, x(1).split(' ').map(x => 255.0 - x.toDouble), x(2))).map{case (label, arr, st) => (label, arr, st)}
...
val trn = trainSet.map(p => LabeledPoint(p.label, scaler.transform(p.features))).persist(DISK_ONLY)
val tst = testSet.map(p => LabeledPoint(p.label, scaler.transform(p.features))).persist(DISK_ONLY)
